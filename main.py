...
from src.data import Dataset, Preprocess, print_shape
...
def main(cfg: KWSConfig) -> None:
    ...
    try:
        ...
        dataset_ = Dataset()
        preprocess_ = Preprocess(dataset_,cfg.paths.train_dir, cfg.params.n_mfcc,
                                cfg.params.mfcc_length, cfg.params.sampling_rate)                       
        preprocessed_dataset: Dataset = preprocess_.preprocess_dataset(preprocess_.labels, 
                                                    cfg.params.test_data_split_percent)
        [print_shape(key, value) for key, value in preprocessed_dataset.__dict__.items()]

        #Loading and training the model
        model: Sequential = CNN_LSTM_Model((cfg.params.n_mfcc, cfg.params.mfcc_length), 
                                        len(preprocess_.labels)).create_model()
        best_selected_model: ModelSelection  = Training(model, preprocessed_dataset, 
